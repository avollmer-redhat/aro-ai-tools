<#
.SYNOPSIS
    List ARO HCP pipeline releases (Azure DevOps builds) for one environment.

.DESCRIPTION
    Wraps the ARO HCP oncall dashboard CLI ('oncall builds') and filters the
    resulting pipeline builds down to a single environment. The environment is
    encoded in the trailing "- <env>" segment of the pipeline definition name,
    e.g. "Incremental - Entrypoint - HCP.Global - stg".

.EXAMPLE
    .\get-releases.ps1 -Environment stg
    .\get-releases.ps1 -Environment test -Hours 6
    .\get-releases.ps1 -Environment prod -Hours 0
#>
...
if ($matched -gt 0) {
    ...
    [Console]::Error.WriteLine("No releases matched environment `"$Environment`".")
    if ($seen.Count -gt 0) {
